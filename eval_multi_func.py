...
class Eval_Multi_Policy(object):
    ...
    def select_action(self, state, rtg, use_diffusion=True, use_ddim=False, normalizer=None):
        ...
        with torch.no_grad():
            ...
            action_c = actions[idx].squeeze()
            action = action_c.cpu().data.numpy().flatten()

            action_list = []
            action_list_c = []

            action_list.append(action)
            action_list_c.append(action_c)
            ...
            for ii in range(h-2):
                ...
                action_ = action__c.cpu().data.numpy().flatten()
            ...
            valueee = self.inner_critic(state[idx, :-1, :], action_list_c.unsqueeze(0))
            value = valueee.squeeze(0)
            for i in range(h-3):
                action_num = -1
                if value[i+1] < value[i+2]:
                    action_num = i+1
                    break
                # inner_value = self.inner_critic(state[idx,i,:].unsqueeze(0), action_list_c[i].unsqueeze(0).unsqueeze(0))
                # if inner_value > pre_value:
                #     action_num = i
                #     break
                # pre_value = inner_value

        new_action_list = action_list[:action_num]
