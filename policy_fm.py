...
class Policy_fm(object):
    def __init__(self,
        ...
        if model == 'transformer':
            ...
            self.flow_model = UncondTemporalUnet(horizon, state_dim, attention=use_attention, dim_mults=dim_mults).to(device)
            ...
            self.flow_model = UncondTemporalUnet(horizon, state_dim, dim=32, attention=use_attention, dim_mults=(1, 2, 4)).to(device)
            self.feasible_generator = FlowMatching(state_dim, model=self.flow_model, device=device, horizon=horizon).to(device)
        elif model == 'flow_mlp_euler':
            self.flow_model = TrajectoryFlowNetwork(horizon, state_dim, hidden_dim=256).to(device)
            self.feasible_generator = FlowMatching_euler(state_dim, model=self.flow_model, device=device, horizon=horizon).to(
                device)
            ...
            self.flow_model = UncondTemporalUnet(horizon, state_dim, attention=use_attention, dim_mults=dim_mults).to(
                device)
            self.feasible_generator = FlowMatching_unet_euler(state_dim, model=self.flow_model, device=device, horizon=horizon).to(
                device)
        elif model == 'expand_mlp_euler':
            self.flow_model = MLP_expand(horizon * state_dim, w=128, time_emb_dim=32).to(device)
            self.feasible_generator = FlowMatching_expand_mlp_euler(state_dim, model=self.flow_model, device=device, horizon=horizon).to(
                device)
        elif model == 'expand_mlp_euler_OT':
            self.flow_model = MLP_expand(horizon * state_dim, w=128, time_emb_dim=32).to(device)
            self.feasible_generator = FlowMatching_expand_mlp_euler_OT(state_dim, model=self.flow_model, device=device, horizon=horizon).to(
                device)
        ...
        # self.discounts = 0.99 ** np.arange(300, dtype=np.float32)[:, None]
    ...
    def evaluate(self, env, eval_episodes=10, normalizer: DatasetNormalizer = None, rtg=None, scale=1.0,
        ...
        for _ in range(eval_episodes):
            ...
            # # for discount rtg
            # rtg = 0.99 ** np.arange(600, dtype=np.float32)
            # rtg = rtg[200:].sum()
            ...
            while not done:
                ...
                # rew.append(reward)
                # discounts = self.discounts[:len(rew)]
                # returns = np.cumsum(discounts * rew)
                # rtg -= returns[-1] / scale
                # rtg = (rtg-reward) / 0.99 / scale  # for discount
    ...
    def train(self, batch):
        ...
        # feasible generator
        # t_loss: torch.Tensor = self.feasible_generator.loss(observations, cond)
        t_loss: torch.Tensor = self.feasible_generator.loss(observations, cond, rtg[:, 0])  # cond_flow
    ...
    def save(self, path=None):
        ...
        torch.save({
            ...
            'ema_model_flow': self.ema_model2.state_dict(),  # ema_flow
    ...
    def load(self, path=None):
        ...
        self.ema_model2.load_state_dict(checkpoint['ema_model2'])  # ema_flow
    ...
    def select_action(self, state, rtg, use_diffusion=True, use_ddim=False, normalizer=None):
        ...
        with torch.no_grad():
            if not use_ddim:
                ...
                state_flow = self.ema_model2(repeat, cond=cond, reward=condtion)  # ema_flow_cond
    ...
    def reset_parameters(self):
        ...
        self.ema_model2.load_state_dict(self.feasible_generator.state_dict())  # ema_flow
    ...
    def step_ema(self, step, step_start_ema):
        ...
        if step % self.update_ema_every == 0:
            ...
            self.ema.update_model_average(self.ema_model2, self.feasible_generator)  # ema_flow


class Policy_fm_doublecritic(object):
    def __init__(self,
                 env_name,
                 state_dim,
                 action_dim,
    ...
    def train(self, batch):
        ...
        rtg_for_critic = rtg.to(self.device)
        observations = observations.to(self.device)
        next_observations = next_observations.to(self.device)
        actions = actions.to(self.device)
        ...
        inner_value = self.inner_critic(observations, actions_for_critic)
        inner_c_loss = F.mse_loss(inner_value, rtg_for_critic.unsqueeze(-1))
        ...
        return {"loss/planner": p_loss.item(),
                ...
                "loss/critic": c_loss.item(),
                "loss/actor": a_loss.item(),
                "loss/inner_critic": inner_c_loss.item(),
                }
    ...
    def save(self, path=None):
        ...
        torch.save({
            ...
            'actor': self.actor.state_dict(),
            'critic': self.critic.state_dict(),
            'inner_critic': self.inner_critic.state_dict(),
