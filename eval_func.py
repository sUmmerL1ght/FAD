...
class Eval_Policy(object):
    ...
    def evaluate(self, env, eval_episodes=10, normalizer: DatasetNormalizer = None, rtg=None, scale=1.0,
        ...
        image_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"image/{env}-FAD/render_img")
        os.makedirs(image_path, exist_ok=True)

        return_to_go = rtg
        for _ in range(eval_episodes):
            state, done = env.reset(), False
            img = Image.fromarray(env.sim.render(width=640, height=480, camera_name="fixed"))
